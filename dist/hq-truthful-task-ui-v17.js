...
const isTransientResourceError=t=>/RESOURCE_EXHAUSTED|quota|rate.?limit|free.?tier|\b429\b/i.test(String(t?.blockedReason||t?.lastFailure?.reason||''));
function createFullTask({employeeId,projectId,details,kind='CUSTOM',priority='NORMAL'}){
  if(!details)return false;const s=read(),e=(s.employees||[]).find(x=>x.id===employeeId);if(!e)return false;
  const t={id:uid('t'),title:details,details,employeeId,projectId,status:'READY',progress:0,kind,priority,createdBy:'NAWAF',createdAt:new Date().toISOString()};
...
document.addEventListener('submit',ev=>{
  ...
  if(!['v8-task-form','v8-command-form','v9-quick-form'].includes(form.id))return;
  ev.preventDefault();ev.stopImmediatePropagation();
  const f=new FormData(form),details=String(f.get('details')||'').trim();if(!details)return;
  const state=read();let employeeId='';
  if(form.id==='v8-task-form'||form.id==='v9-quick-form')employeeId=String(f.get('employeeId')||'');
  ...
  if(createFullTask({employeeId,projectId,details,kind,priority})){
    form.closest('#v8-modal,#v9-modal')?.querySelector('[data-close],[data-v9-close]')?.click();
  }
},true);
function repairState(){
  ...
  for(const t of tasks){
    ...
    if(t?.status==='WAITING_FOR_NAWAF'&&isTransientResourceError(t)){t.status='PAUSED';t.pauseReason='EXTERNAL_RESOURCE_LIMIT';changed=true;const e=(s.employees||[]).find(x=>x.id===t.employeeId);if(e&&e.status==='WAITING_FOR_NAWAF'){e.status='PAUSED';e.task=t.details||t.title;changed=true}}
  }
  ...
  if(changed){repairing=true;write(s,'truthful-task-state-repair');setTimeout(()=>{repairing=false},120)}
}
...
function enhance(){
  ...
  document.querySelectorAll('.v8-row[data-task]').forEach(row=>{
    ...
    if(info&&t.status==='PAUSED'&&isTransientResourceError(t)&&!info.dataset.resourceEnhanced){info.textContent+=' • لا يحتاج قرار منك؛ يعاد عند توفر الحصة';info.dataset.resourceEnhanced='1'}
  });
