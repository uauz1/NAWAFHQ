...
function sanitize(s){
 ...
 for(const e of (s.employees||[])){
   if(e?.disabled||e?.deletedAt){e.status='PAUSED';e.task='';continue}
...
function stamp(o){if(!o||typeof o!=='object')return 0;let best=0;for(const k of ['updatedAt','lastRunAt','executionFinishedAt','executionStartedAt','approvedAt','rejectedAt','createdAt']){const t=Date.parse(o[k]||'');if(Number.isFinite(t)&&t>best)best=t}return best}
...
function newestObject(remote={},local={},preferRemote=false){remote=remote&&typeof remote==='object'?remote:{};local=local&&typeof local==='object'?local:{};const rs=stamp(remote),ls=stamp(local);if(rs||ls)return ls>rs?{...remote,...local}:{...local,...remote};return preferRemote?{...local,...remote}:{...remote,...local}}
function mergeState(remote={},local={},preferRemoteScalars=false){const out=preferRemoteScalars?{...local,...remote}:{...remote,...local};for(const k of ['employees','projects','tasks','reports','activity','approvals','meetings','goals','connectionRequests','secretaryBriefs','notifications','archives'])out[k]=mergeList(remote[k],local[k]);out.settings=newestObject(remote.settings,local.settings,preferRemoteScalars);out.projectMemory=newestObject(remote.projectMemory,local.projectMemory,preferRemoteScalars);out.secretarySeen=[...new Set([...(remote.secretarySeen||[]),...(local.secretarySeen||[])])].slice(0,250);out.trading=newestObject(remote.trading,local.trading,preferRemoteScalars);out.finance=newestObject(remote.finance,local.finance,preferRemoteScalars);if(remote.finance?.watchlist||local.finance?.watchlist){const w=new Map();for(const x of [...(remote.finance?.watchlist||[]),...(local.finance?.watchlist||[])])if(x?.symbol){const key=String(x.symbol).toUpperCase(),old=w.get(key);w.set(key,!old||stamp(x)>=stamp(old)?x:old)}out.finance.watchlist=[...w.values()]};return sanitize(out)}
