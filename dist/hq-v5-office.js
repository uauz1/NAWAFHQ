...
const start=()=>setTimeout(boot,100);document.readyState==='loading'?document.addEventListener('DOMContentLoaded',start):start();window.addEventListener('nawaf:state-updated',()=>{const old=document.querySelector('.hq-3d-stage');if(old){old.dataset.sceneReady='';old.querySelector('.office-scene')?.remove();setTimeout(boot,80)}})
})();