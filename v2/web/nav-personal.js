...
function bridgeToken(){let token=localStorage.getItem('nav_bridge_token')||'';if(token)return token;token=prompt('الصق رمز NAV Bridge الموجود داخل config.json أو README_AR.txt:','')?.trim()||'';if(token)localStorage.setItem('nav_bridge_token',token);return token;}
async function runPcTarget(target){const online=await checkBridge();if(!online){addLocalMessage('ما قدرت أوصل لجسر ناڤ على الكمبيوتر. شغّل start-nav-bridge.bat أول، وبعدها جرّب نفس الأمر.');return false;}const token=bridgeToken();if(!token){addLocalMessage('الجسر شغال، بس باقي رمز الربط. اضغط «ربط» والصق الرمز الموجود مع NAV Bridge.');return false;}try{const r=await bridgeFetch('/command',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,action:'open',target:target.id})},3500);const d=await r.json();if(!r.ok||!d?.ok)throw new Error(d?.error||'BRIDGE_COMMAND_FAILED');addLocalMessage(`تم تنفيذ الأمر على الكمبيوتر: فتح ${target.label}.`);return true;}catch(error){if(String(error.message).includes('UNAUTHORIZED')){localStorage.removeItem('nav_bridge_token');addLocalMessage('رمز NAV Bridge غير صحيح. اضغط «ربط» وحط الرمز من جديد.');}else addLocalMessage('الجسر متصل، لكن تنفيذ الأمر فشل. ما راح أقول إنه تم إلا إذا رجع تأكيد حقيقي من الكمبيوتر.');return false;}}
async function interceptPcShortcut(event){const input=$('#messageInput'),text=input?.value?.trim();if(!text)return;const target=pcTargetFrom(text);if(!target)return;event.preventDefault();event.stopImmediatePropagation();input.value='';addLocalMessage(`أرسل للكمبيوتر: افتح ${target.label}…`);await runPcTarget(target);}
async function connectPc(){const online=await checkBridge();if(!online){alert('NAV Bridge مو شغال على هذا الكمبيوتر. شغّل start-nav-bridge.bat ثم اضغط ربط مرة ثانية.');return;}const current=localStorage.getItem('nav_bridge_token')||'';const token=prompt('الصق رمز NAV Bridge من config.json أو README_AR.txt:',current)?.trim();if(token){localStorage.setItem('nav_bridge_token',token);setPcStatus(true,'الجسر شغال ورمز الربط محفوظ على هذا المتصفح.');}}
