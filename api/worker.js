...
async function gemini(task,state){
 ...
 const body={system,messages:[{role:'user',text:JSON.stringify({task:{title:task.title,details:task.details||task.title},employee:e?{name:e.name,role:e.role}:null,project:p?{name:p.name,phase:p.phase}:null})}]};
 const r=await fetch('https://generativelanguage.googleapis.com/v1beta/models/'+encodeURIComponent(MODEL)+':generateContent',{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':process.env.GEMINI_API_KEY},body:JSON.stringify({systemInstruction:{parts:[{text:system}]},contents:body.messages.map(m=>({role:'user',parts:[{text:m.text}]})),generationConfig:{temperature:.35,maxOutputTokens:1800,responseMimeType:'application/json'}})});
...
async function processOne(state){
 const task=(state.tasks||[]).find(t=>t.status==='READY');if(!task)return false;
 ...
 let result;try{result=await gemini(task,state)}catch(err){task.status='READY';task.blockedReason='تعذر تشغيل AI مؤقتًا';if(e)e.status='READY';activity(state,'تعذر تشغيل المهمة مؤقتًا: '+task.title);await saveState(state);throw err}
...
export default async function handler(req,res){
 ...
 if(process.env.CRON_SECRET&&req.headers.authorization!==('Bearer '+process.env.CRON_SECRET))return send(res,401,{ok:false,error:'UNAUTHORIZED'});
 try{let state=await getState();if(!state.companyStarted)return send(res,200,{ok:true,processed:0,reason:'COMPANY_NOT_STARTED'});let n=0;for(let i=0;i<3;i++){if(!(await processOne(state)))break;n++;state=await getState()}return send(res,200,{ok:true,processed:n})}catch(e){console.error('worker',e?.message||e);return send(res,500,{ok:false,error:'WORKER_ERROR'})}
}