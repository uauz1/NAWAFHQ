...
const mobileShellV25=read('hq-mobile-shell-v25.css');
...
if(!appsHtml.includes('linear-gradient(145deg,rgba(15,21,31,.94),rgba(8,12,18,.9))'))throw new Error('Approved premium apps visual baseline changed unexpectedly');
if(!html.includes('hq-ceo-v13.js?v=20260918-project-approval-2'))throw new Error('CEO approval runtime is not activated in production index');
if(!ceoV13.includes('px.hasChanges||px.changed'))throw new Error('CEO approval misses changed project proposals');
if(!ceoV13.includes('t.updatedAt=t.approvedAt'))throw new Error('CEO approval is not timestamped for cloud conflict resolution');
if(!ceoV13.includes('t.updatedAt=t.rejectedAt'))throw new Error('CEO rejection is not timestamped for cloud conflict resolution');
...
if(!app.includes(".v8-row[data-task]"))throw new Error('Task rows lack robust click handling');
if(!html.includes('hq-shell-v8.js?v=20260918-task-click-stability-1'))throw new Error('Task click stability patch is not activated in production index');
for(const editFeature of ['v8-employee-edit-form','employee-updated','المهام / المسؤوليات الأساسية','x.updatedAt=new Date().toISOString()'])if(!app.includes(editFeature))throw new Error(`Missing employee edit feature ${editFeature}`);
...
if(!ceoV13.includes('requiresCommitEvidence:true'))throw new Error('CEO approval does not persist commit-evidence requirement for project changes');
if(!ceoV13.includes('delete t.executionApproval'))throw new Error('CEO rejection does not revoke project execution approval');
...
if(!secretary.includes('exec-evidence'))throw new Error('Executive secretary does not render execution evidence');
