...
for (const asset of ["hq-v5.css", "hq-v5.js", "hq-v5-office.js", "hq-workforce-policy.js", "three@0.160.1"]) {
...
for (const rule of ["مهمة مستقلة", "دليل محفوظ", "لا يتم اختلاق نشاط أو تقدم أو نتيجة", "لا يتم إنشاء أي التزام مالي"]) {
  if (!workforce.includes(rule)) throw new Error(`Missing workforce rule: ${rule}`);
}
for (const fn of ["create", "addEvidence", "canComplete", "normalize"]) {
  if (!workforce.includes(fn)) throw new Error(`Missing workforce capability: ${fn}`);
}

for (const asset of ["hq-v5.css", "hq-app-integrations.css", "hq-apps-page.js"]) {
  if (!appsPage.includes(asset)) throw new Error(`Missing apps page asset: ${asset}`);
}
for (const link of ["mueen-islamic-app.vercel.app", "qaddha.vercel.app", "github.com/uauz1/mueen-islamic-app", "github.com/uauz1/game"]) {
  if (!appsScript.includes(link)) throw new Error(`Missing linked app resource: ${link}`);
}
for (const feature of ["توجيه موظف", "لوحة تحكم", "تقرير سريع"]) {
  if (!appsPage.includes(feature) && !appsScript.includes(feature)) throw new Error(`Missing apps feature: ${feature}`);
}
