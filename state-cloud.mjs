...
export async function handleStateApi(req,res){
 try{
  if(req.method==='GET'){
   const rows=await request('hq_state?id=eq.main&select=data,updated_at');
   return send(res,200,{ok:true,state:rows?.[0]?.data||null,updatedAt:rows?.[0]?.updated_at||null});
  }
  if(req.method==='POST'){
