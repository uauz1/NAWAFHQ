...
function mergeProtectedState(current={},incoming={}){const out={...current,...incoming};for(const k of ['employees','projects','tasks','reports','activity','approvals','meetings','goals'])out[k]=mergeList(current[k],incoming[k]);out.settings={...(current.settings||{}),...(incoming.settings||{})};return out}
...
export async function handleStateApi(req,res){
 try{
  ...
  if(req.method==='POST'){
   ...
   try{const saved=await saveCloudState(b.state,b.baseUpdatedAt||'');return send(res,200,{ok:true,updatedAt:saved.updatedAt})}catch(e){if(e?.message==='STATE_CONFLICT')return send(res,409,{ok:false,error:'STATE_CONFLICT',state:e.current?.state,updatedAt:e.current?.updatedAt});throw e}
  }
